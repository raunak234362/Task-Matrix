...
const GanttChart = ({ taskData }) => {
                ...
                <div key={member.id} className="flex mt-2 h-10 relative">
                  <div className="flex-grow relative">
                    ...
                    {member.tasks.map((task) => {
                      ...
                      const taskDuration =
                        differenceInDays(parseISO(task.due_date || addDays(parseISO(task.created_on), 30)), parseISO(task.created_on)) + 1;
                      ...
                      return (
                        <div
                          ...
                          title={`${task.name} (${format(parseISO(task.created_on), 'MMM dd')} - ${
                            task.due_date ? format(parseISO(task.due_date), 'MMM dd') : 'No End Date'
                          })`}
                        >
  ...
  {taskData && taskData.length > 0 ? (
    taskData.map((task) => {
      const startDate = isValidDate(task?.tasks[0]?.project?.created_at)
        ? format(parseISO(task?.tasks[0]?.project?.created_at), 'yyyy-MM-dd')
        : 'Invalid Start Date';
      const endDate = isValidDate(task?.tasks[0]?.project?.endDate)
        ? format(parseISO(task?.tasks[0]?.project?.endDate), 'yyyy-MM-dd')
        : 'No End Date';
