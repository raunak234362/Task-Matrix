        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div className="bg-white shadow-lg rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <h2 className="text-2xl font-bold text-gray-900">{format(firstDayCurrentMonth, "MMMM yyyy")}</h2>
              <button
                type="button"
                onClick={nextMonth}
                className="flex items-center justify-center p-2 rounded-md bg-gray-200 hover:bg-gray-300"
                ...
                <div key={day.toString()} className={classNames(dayIdx === 0 && colStartClasses[getDay(day)], "py-1.5")}>
                  <button
                    ...
                    className={classNames(
                      ...
                      !isEqual(day, selectedDay) && !isToday(day) && isSameMonth(day, firstDayCurrentMonth) && "bg-white text-gray-900",
                      !isEqual(day, selectedDay) && !isToday(day) && !isSameMonth(day, firstDayCurrentMonth) && "bg-gray-100 text-gray-400",
...
function PriorityButton({ level, currentLevel, setLevel }) {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 border-2 ${priorityColors[level].bg} ${priorityColors[level].border} ${priorityColors[level].text} rounded-full`}></span>
      <span className={`ml-2 ${priorityColors[level].text}`}>{currentLevel === level ? "All" : ["Low Priority", "Medium Priority", "High Priority", "Critical Priority"][level]}</span>
    </div>
  );
}
...
function Task({ task }) {
  ...
  return (
    <li className={`border-2 p-4 rounded-lg ${priorityColors[task.priority].bg} ${priorityColors[task.priority].border} ${priorityColors[task.priority].text}`}>
      <div className="flex justify-between items-center font-bold">
        <span>Project: <span className="font-medium">{task.project.name}</span></span>
        <span>Task: <span className="font-medium">{task.name}</span></span>
      </div>
      <div className="flex justify-between items-center font-bold">
        <span>Status: <span className="font-medium">{task.status}</span></span>
        <span>Priority: <span className="font-medium">{["Low", "Medium", "High", "Critical"][task.priority]}</span></span>
      </div>
      <div className="font-medium">
        <span><strong>Deadline:</strong> {new Date(task.due_date).toDateString()}</span>
      </div>
    </li>
  );
}
